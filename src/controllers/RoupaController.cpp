...
void RoupaController::lerRoupa(){
  ...
  while(loop){
    try {
      ...
      if(!dao.existeEssaRoupa(nome)) // Verificar se roupa foi criada e lancar throw expcetion
        throw std::runtime_error("Erro: Essa roupa não existe. Quer fazer a operação novamente?(y/n) ");
